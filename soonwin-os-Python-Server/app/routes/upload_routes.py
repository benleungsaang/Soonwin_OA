...
def upload_file():
    ...
    try:
        ...
        if target_path:
            ...
            save_path, relative_path, unique_filename = save_uploaded_file(file, full_target_path, use_date_subdir=False)
        else:
            # 否则保存到临时位置
            temp_path = os.path.join(current_app.root_path, '..', UPLOAD_CONFIG['TEMP_UPLOAD_FOLDER'])
            save_path, relative_path, unique_filename = save_uploaded_file(file, temp_path, use_date_subdir=False)

        # 返回文件路径信息
        full_relative_path = os.path.relpath(save_path, os.path.join(current_app.root_path, '..'))
