  ...
  <div class="order-management-container">
    ...
    <el-card shadow="hover" class="expense-summary-card" style="margin-bottom: 20px;">
      ...
      <div class="summary-content" v-if="expenseSummary">
        <h4>{{ expenseSummary.year }}年度费用汇总</h4>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
          <div>
            <strong>年度目标: </strong>
            <span>￥ {{ formatCurrency(expenseSummary.annual_target) }}</span>
            <el-button size="small" @click="showAnnualTargetDialog" style="margin-left: 10px;">修改年度目标</el-button>
          </div>
        </div>
        <el-table :data="[expenseSummary]" style="width: 100%" border>
          ...
          <el-table-column prop="total_expenditure" label="年度总开支" width="150">
            <template #default="scope">
              <span :class="scope.row.total_expenditure >= 0 ? 'negative' : 'positive'">
                {{ formatCurrency(scope.row.total_expenditure || 0, 'css') }}
              </span>
            </template>
          </el-table-column>
          <el-table-column prop="total_income" label="年度总收入" width="150">
            <template #default="scope">
              <span :class="scope.row.total_income >= 0 ? 'negative' : 'positive'">
                {{ formatCurrency(scope.row.total_income || 0, 'css') }}
              </span>
            </template>
          </el-table-column>
          <el-table-column prop="total_expenses" label="合计分摊费用" width="150">
            <template #default="scope">
              <span :class="scope.row.total_expenses >= 0 ? 'negative' : 'positive'">
                {{ formatCurrency(scope.row.total_expenses, 'css') }}
              </span>
            </template>
          </el-table-column>
          <el-table-column prop="total_orders" label="订单数量" width="120">
            <template #default="scope">
              <span :class="scope.row.total_orders >= 0 ? 'negative' : 'positive'">
                {{ scope.row.total_orders }}
              </span>
            </template>
          </el-table-column>
          <el-table-column prop="total_order_amount" label="订单总费用" width="150">
            <template #default="scope">
              <span :class="scope.row.total_order_amount >= 0 ? 'negative' : 'positive'">
                {{ formatCurrency(scope.row.total_order_amount, 'css') }}
              </span>
            </template>
          </el-table-column>
          <el-table-column prop="net_profit_estimate" label="扣除分摊后净利" width="180">
            <template #default="scope">
              <span :class="scope.row.net_profit_estimate >= 0 ? 'negative' : 'positive'">
                {{ formatCurrency(scope.row.net_profit_estimate, 'css') }}
              </span>
            </template>
          </el-table-column>
        </el-table>
        <div style="margin-top: 10px; text-align: right;">
          <el-button type="primary" @click="updateOrderProportionateCost">更新订单摊分费用</el-button>
        </div>
      </div>
    ...
    <el-card shadow="hover" class="management-card">
      ...
      <el-form :model="searchForm" :inline="true" class="search-form">
        ...
        <el-form-item label="名称">
          <el-input v-model="searchForm.machineName" placeholder="请输入名称" clearable></el-input>
        </el-form-item>
      ...
      >
        ...
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="scope">
            ...
            <el-button size="small" @click="showIndividualExpensesDialog(scope.row)">添加个别费用</el-button>
    ...
    >
      ...
      >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="运营成本" prop="operational_cost">
              <el-input-number
              v-model="orderForm.operational_cost"
              :precision="2"
              :min="0"
              style="width: 100%"
              :format="formatNumber"
    ...
    >
      <div class="individual-expenses-header">
        <h4>订单信息：{{ currentOrder.customer_name }} (合同金额: ¥{{ formatCurrency(currentOrder.contract_amount) }})</h4>
        <div class="individual-expenses-actions">
          <el-button type="primary" @click="showAddIndividualExpenseDialog">添加个别费用</el-button>
        </div>
      </div>
        ...
        <el-table-column prop="amount" label="费用金额" width="120">
          <template #default="scope">
            <span :class="scope.row.amount >= 0 ? 'positive' : 'negative'">
              {{ formatCurrency(scope.row.amount || 0, 'css') }}
            </span>
          </template>
        </el-table-column>
      ...
      <div class="individual-expenses-total" style="margin-top: 20px; padding: 15px; background-color: #f5f7fa; border-radius: 4px;">
        <strong>个别费用总计：¥{{ formatCurrency(individualExpensesTotal, 'css') }}</strong>
      </div>
...
// 格式化货币显示
const formatCurrency = (value: number, css:'') => {
  if (value === null || value === undefined) return '0.00';
  const formattedValue = Math.abs(Number(value)).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  if (css){
...
const calculateProfits = () => {
  // 使用回款作为计算基础，如果没有回款则使用合同金额
  const baseAmount = orderForm.value.payment_received || orderForm.value.contract_amount;

  // 毛利 = 回款（或合同金额） - 机器成本 - 佣金
  const grossProfit = baseAmount - orderForm.value.machine_cost - orderForm.value.commission;
  orderForm.value.gross_profit = grossProfit >= 0 ? grossProfit : 0;  // 确保毛利非负

  // 净利 = 回款（或合同金额） - 机器成本 - 佣金 - 运营成本
  const netProfit = baseAmount - orderForm.value.machine_cost - orderForm.value.commission - orderForm.value.operational_cost;
  orderForm.value.net_profit = netProfit >= 0 ? netProfit : 0;  // 确保净利非负
};
...
const showAddIndividualExpenseDialog = () => {
  individualExpenseDialogTitle.value = '添加个别费用';
